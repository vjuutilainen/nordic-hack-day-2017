...
df_ideologies_shorted <- df_ideologies %>% rowwise() %>% mutate(short = shorten_group_name(EPgroupLabel))

df_ideologies_shorted
...
```{r}
m <- leaflet(data=mapdata %>% filter(politicalGroup=='Group of the European People\'s Party (Christian Democrats)')) %>%
  addTiles() %>%  # Add default OpenStreetMap map tiles
  addCircleMarkers(
    radius = 5,
    color = ~pal(politicalGroup),
...
```{r}
m <- leaflet(data=mapdata %>% filter(politicalGroup=='Confederal Group of the European United Left - Nordic Green Left')) %>%
  addTiles() %>%  # Add default OpenStreetMap map tiles
  addCircleMarkers(
    radius = 5,
    color = ~pal(politicalGroup),
...
```{r}
m <- leaflet(data=mapdata %>% filter(politicalGroup=='Europe of Nations and Freedom Group')) %>%
  addTiles() %>%  # Add default OpenStreetMap map tiles
  addCircleMarkers(
    radius = 5,
    color = ~pal(politicalGroup),
